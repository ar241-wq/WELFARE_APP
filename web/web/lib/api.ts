...
export async function createChallenge(payload: { title: string; description: string; reward_credits: number; deadline?: string }) {
...
export async function distributePrize(challengeId: number, departmentId: number) {
  const { data } = await api.post(`/api/challenges/${challengeId}/distribute/`, { department_id: departmentId });
  return data;
}
